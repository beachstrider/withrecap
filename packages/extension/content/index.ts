...
class GoogleMeetsService implements MeetingService {
  ...
  updateMeetingMetadata(endCall: boolean = false): void {
    const nbParticipants = document.querySelector(
      this.SELECTOR_PARTICIPANTS
    )?.textContent;
  ...
  startTranscribing(): void {
    ...
    const endCallDiv = document.querySelector(
      this.SELECTOR_END_CALL
    ) as HTMLElement;
    const self = this;
    if (!ccDiv) {
    ...
    console.log(
      'found caption div - checking if hidden: ' + ccDiv.style.display
    );
    if (ccDiv.style.display === 'none') {
      console.log("caption isn't enabled - enabling");
      const ccButton = callDiv.querySelector(
        this.SELECTOR_CC_BUTTON
      ) as HTMLElement;
      if (ccButton) {
