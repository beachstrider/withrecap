...
class GoogleMeetsService implements MeetingService {
    ...
    updateMeetingMetadata(endCall: boolean = false): void {
        ...
        chrome.runtime.sendMessage({ 'meetingId': this.getMeetingId(), 'metadata': metadata, 'type': ExtensionMessages.MeetingMetadata }, () => { });
    }
    startTranscribing(): void {
        ...
        const ccDivObserver = new MutationObserver(() => {
            ...
            chrome.runtime.sendMessage({ 'meetingId': this.getMeetingId(), 'message': meetingMessage, 'type': ExtensionMessages.MeetingMessage }, () => { });
        });
        ...
        participantsDivObserver.observe(participantsDiv, { characterData: true, attributes: true, childList: true, subtree: true });        
